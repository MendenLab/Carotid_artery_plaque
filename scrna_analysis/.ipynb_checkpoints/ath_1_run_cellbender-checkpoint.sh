...
ref_genome_list=("GRCh38-p13-Gencode_v33")
intron_status_list=("with_intron") #,"without_intron")
...
for intron_status  in ${intron_status_list[@]};do
    ...
    for ref_genome in ${ref_genome_list[@]};do
        ...
        for batch in ${batch_list[@]}; do
            ...
            output_dir="${base_dir}/data/cellbender_output/${ref_genome}/${intron_status}/${batch}"
            mkdir -p "${base_dir}/data/cellbender_output/${ref_genome}/${intron_status}/${batch}"
            ...
            for sample_name in ${sample_list[@]}; do
                input_file="${base_dir}/data/raw_counts/${intron_status}/${ref_genome}_${batch}_${sample_name}_raw.h5ad"
                    ...
                    if [ -e "$input_file" ]; then
                        ...
                        echo "Starting ${intron_status} ${ref_genome} ${batch} ${sample_name}"
                        #echo "$output_file"
                    
                        cd "${base_dir}/data/cellbender_output/${ref_genome}/${intron_status}/${batch}"
