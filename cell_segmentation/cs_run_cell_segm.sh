...
### SETUP DATA FOLDER TO LOOP OVER ==> CHANGE data_dir TO CUSTOM FOLDER WHERE RAW XENIUM DATA IS STORED
## SET BASE DIRECTORY ==>
#. ==> SHOULD CONTAIN FOLLOWING FOLDERS: 
##. ==> xenium_data: directory containing RAW Xenium data, unzipped, with .TIFF files of Xenium slide images
...
#. => 10x: take the default 10x segmentations as output from Xenium Onboard Analyzer
#. => cellpose: run cellpose cell segmentation, with 2 model types
    ...
    for sample_name in "${sample_name_list[@]}"; do
    	...
    	if [[ -d "$sample_dir" ]]; then #&& [[ "$sample_dir" == *"P3_H"* ]]
            ...
            if [ "$segm_method" == "10x" ]; then
                ...
                python util_create_10x_nuclear_masks.py -i $input_fn -o $output_dir -n $nucleus_bound_fn #-ow # if cell mask alrady exists, overwrite it
