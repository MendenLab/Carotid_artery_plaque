...
start_baysor_docker_container() {
    ...
    ## SET BASE DIRECTORY ==>
    #. ==> SHOULD CONTAIN FOLLOWING FOLDERS: 
    ##. ==> xenium_data: (directory containing RAW Xenium data, unzipped)
    ...
    ### DOCKER IMAGE NAME
    ## 1. Pull docker image with following line: 
    #.    docker pull gargerd/baysor:v0.6.2
    ## 2. Set image name of pulled docker image as "dock_img_name" variable
    ...
    bash ${current_dir}/bays_stop_active_baysor_docker.sh
    
    ## Run docker with baysor image
    sudo docker run -it --name baysor_docker -d \
                --mount type=bind,source=$base_dir,target=$base_dir -w ${current_dir} \
                $dock_img_name bash 
}
...
run_baysor_cli(){
    ...
    sudo docker exec baysor_docker python3.10 util_run_baysor.py -m $baysor_molecules \
                                                            -d $baysor_data_dir \
                                                            -s $segm_method \
                                                            -id $id_code \
    ...
    sudo docker exec baysor_docker python3.10 util_run_baysor_preview.py -m $baysor_molecules \
                                                                    -d $baysor_data_dir \
                                                                   --temp $baysor_output_fold #\
                                                                   #-p $hyperparams 
    fi                                                                    
...
if [ "$dataset_type" == "atherosclerosis" ]; then
    ...
    ## ==> code_folder: root folder for code (name can be changed)
    base_dir=$(dirname "$(dirname "$PWD")")
    data_dir="${base_dir}/xenium_data"
    
    # Drop scratch folders that start with "._"
    panel_dir_list=($(find "$data_dir" -maxdepth 1 -type d -name '*Panel2*' -a ! -name '._*' | sort))
    sample_name_list=('P1_H' 'P1_D' 'P2_H' 'P2_D' 'P3_H' 'P3_D' 'P4_H' 'P4_D' 'P5_D' 'P6_D' 'P7_D' 'P8_D' 'P9_D' 'P10_D' 'P11_D' 'P12_D') #
 
fi    
    
if [ "$dataset_type" == "breast_cancer" ]; then

    ## TO RUN BAYSOR ON PUBLICLY AVAILABLE BREAST CANCER DATA, DOWNLOAD IT FROM: 
    #. https://www.10xgenomics.com/products/xenium-in-situ/preview-dataset-human-breast
    #.  AND COPY IT INTO xenium_data/breast_data
    
    base_dir=$(dirname "$(dirname "$PWD")")
    data_dir="${base_dir}/xenium_data/breast_data"
                    ...
                    if [ "$segm_method" = "cellpose" ]; then
                        ...
                        for cp_model_type in "${cellpose_model_types[@]}"; do
                            for cp_expans_size in "${segment_expansion_sizes_um[@]}"; do
                                ...
                                sudo docker exec baysor_docker python3.10 util_preprocess_transcripts.py -d $sample_dir  -p $cell_segm_dir -s $segm_method#-ow  #if file already exists, overwrite
            
                                ## 3. Execute util_run_baysor.py file or util_run_baysor_preview.py file
